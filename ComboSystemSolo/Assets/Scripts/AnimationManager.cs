...
public class AnimationManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //bool setThis = (horizontalInput > 0.2f || horizontalInput < -0.2f);
        player.GetComponent<Animator>().SetBool("IsWalking", horizontalInput >= 0.2f || horizontalInput <= -0.2f);
        if (horizontalInput > 0.1f)
            player.GetComponent<SpriteRenderer>().flipX = false;
        else if (horizontalInput < -0.1f)
            player.GetComponent<SpriteRenderer>().flipX = true;
        //player.GetComponent<Animator>().SetBool("DirectionLeft", left);
        // Debug.Log(player.GetComponent<Animator>().GetBool("DirectionLeft"));
        //Debug.Log(player.GetComponent<Animator>().GetFloat("Walking"));
